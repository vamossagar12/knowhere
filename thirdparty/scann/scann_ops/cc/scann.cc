...
std::shared_ptr<uint8_t[]> GetBinary(google::protobuf::Message* message) {
    auto bin = new uint8_t[message->ByteSizeLong()];
    message->SerializeToArray(bin, message->ByteSizeLong());
    return std::shared_ptr<uint8_t[]>(bin);
}

::google::protobuf::MessageLite GetProto(const void* data, int size) {
    ::google::protobuf::MessageLite message;
    message.ParseFromArray(data, size)
    return message;
}

size_t GetSize(google::protobuf::Message* message) {
